...
func BenchmarkResponseValue_Read(b *testing.B) {

	for _, entry := range benchRespReadTable {
		b.Run(entry.name, func(b *testing.B) {
			value := new(ResponseValue)
			b.ResetTimer()

			for i := 0; i < b.N; i++ {
				_, _ = value.Read(entry.data, entry.object)
			}
		})
	}
}
